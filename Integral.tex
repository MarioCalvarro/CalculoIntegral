...
$$f = \sup_{k\in \mathbb{N}} \{f_k\} \text{ es medible}$$
\end{prop}
\begin{demo}
\begin{itemize}
...
\begin{coro}
Sea $A \subset \mathbb{R}^n$ medible y sea $\{f_k\}_{k=1}^\infty$ una sucesión de funciones donde $\forall k \in \mathbb{N}: f_k: A \rightarrow \left[-\infty, \infty\right]$ es medible, entonces:
$$f = \inf_{k\in \mathbb{N}} \{f_k\} \text{ es medible}$$
\end{coro}
\begin{demo}
Basta ver que $f = \inf f_k = -\sup_k \left(-f_k\right)$
\end{demo}
...
\begin{prop}
Si $f$ y $g$ son dos funciones tales que son iguales en casi todo punto y una de las dos es medible, entonces la otra lo es.
$$f\mbox{ medible y } f=g \mbox{ (c.t.p.)}\Rightarrow g \mbox{ medible}$$
\end{prop}
\begin{demo}
...
\begin{demo}
Observamos que: 
$$f\cdot \chi_{\left[-N, N\right]^{n}} \rightarrow_{N \rightarrow \infty} f$$
...
¿$\varphi_k \rightarrow^k f\cdot \chi_{\left[-N, N\right]}$ ctp?

Si $f$ es continua en $x  \in \left[-N, N\right]^n\Rightarrow$
$$\{\varphi_k\left(x\right)\}_k \rightarrow f\left(x\right) \Rightarrow f\left(x\right) = \lim_{k \rightarrow \infty}\varphi_k\left(x\right)$$
...
Sea $k_0: Q_j^{k_0}$ que contiene a $x$, verifica $Q_j^{k_0} \subset B\left(x, \delta\right) \Rightarrow$
$$\forall k \ge k_0,\ Q_j^k \subset B\left(x, \delta\right) \Rightarrow$$
$$\forall y \in Q_j^k,\ \vert\vert y - x\vert\vert < \delta \Rightarrow \vert f\left(y\right) - f\left(x\right) \vert < \varepsilon \Rightarrow \vert c_j^k - f\left(x\right) \vert \le \varepsilon \text{ donde } c_j^k = \varphi_k\left(x\right) \Rightarrow$$
...
(\textit{Dibujo}) 
$$\sum_{I \in P_1} \mu\left(I\right) < \varepsilon \; \land \; D_{\frac{1}{n}} \subset \bigcup_{I \in P_1} I \Rightarrow$$
...
\begin{itemize}
    ...
    $$\varphi = \sum_{k=1}^{r} \beta_j \aleph{E_j} \Rightarrow \varphi \text{ es simple.}$$
\end{itemize}

\begin{defi}[Integral]
Sea $\varphi$ función simple no negativa:
$$\varphi = \sum_{k=1}^{m} \alpha_k \chi{E_k} \mbox{ donde } \left(\alpha_k > 0\right)$$
...
\underline{Obs}:
Asumimos que $0 \cdot \left(+ \infty\right) = 0 \Rightarrow I\left(\varphi\right) \in \left[0, +\infty\right]$
...
\begin{enumerate}
    ...
    \item $\varphi, \psi$ simples no negativas $\Rightarrow I\left(\varphi + \psi\right) = I\left(\varphi\right) + I\left(\psi\right)$
    \item $\varphi \le \psi$ c.t.p $\Rightarrow I\left(\varphi\right) \le I\left(\psi\right)$
    \item $\varphi = \psi$ c.t.p $\Rightarrow I\left(\varphi\right) = I\left(\psi\right)$
\end{enumerate}
\end{prop}
\begin{demo}
...
\begin{defi}
Sea $f: \mathbb{R}^n \rightarrow \left[0, +\infty\right]$ medible. Definimos como \textbf{integral (Lebesgue) de $f$} a:
$$\int_{\mathbb{R}^n}f d\mu = \sup_{\substack{\varphi \le f \text{ c.t.p} \\ \varphi \text{ simple}}} I\left(\varphi\right) \in \left[0, +\infty\right]$$
\end{defi}

\begin{prop}
Sea $\varphi$ simple no negativa $\Rightarrow$
$$\int_{\mathbb{R}^n}\varphi d \mu = I\left(\varphi\right)$$
\end{prop}
\begin{demo}
$$\varphi \le \varphi \Rightarrow I\left(\varphi\right) \le \int_{\mathbb{R}^n} \varphi d \mu$$
Si $\int_{\mathbb{R}^n} \varphi d \mu = +\infty \Rightarrow$
$$\forall M > 0,\ \exists \psi \le \varphi: I\left(\psi\right) \ge M \Rightarrow$$
$$M \le I\left(\psi\right) \le I\left(\varphi\right) \Rightarrow I\left(\varphi\right) \ge M,\ \forall M \Rightarrow I\left(\varphi\right) = +\infty$$
Si $\int_{\mathbb{R}^m}\varphi d \mu < +\infty \Rightarrow$
$$\forall \varepsilon > 0,\ \exists \psi \le \varphi: I\left(\psi\right) \ge \int_{\mathbb{R}^n}\varphi d \mu - \varepsilon \Rightarrow$$
$$I\left(\varphi\right) \ge \int_{\mathbb{R}^n}\varphi d \mu - \varepsilon \Rightarrow I\left(\varphi\right) \ge \int_{\mathbb{R}^n}\varphi d \mu$$
\end{demo}

\begin{prop}
Sea $f, g: \mathbb{R}^n \rightarrow \left[0, +\infty\right]$ medibles tal que $f \le g$ c.t.p. Entonces:
...
\begin{coro}
Si $f = g$ c.t.p $\Rightarrow$
$$\int_{\mathbb{R}^n}f d \mu = \int_{\mathbb{R}^n}g d \mu$$
\end{coro}

\begin{defi}
Sean $A \subset \mathbb{R}^n$ medible, $f: A \rightarrow \left[0, +\infty\right]$ medible:
