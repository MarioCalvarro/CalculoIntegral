...
-- INICIO CLASES TEÓRICAS CON JESÚS JARAMILLO --

\textbf{NOTACIÓN:} (Creo que esto lo podemos quitar)

En el espacio $\mathbb{R}^n \times \mathbb{R}^n = \mathbb{R}^{n+m}$, sea la función $f: \mathbb{R}^n \times \mathbb{R}^m \rightarrow \left[-\infty, \infty\right]$.
...
\item La función $F: \mathbb{R}^n \rightarrow \left[0, \infty\right]$ definida en casi todo punto por:
...
\end{enumerate}
Este teorema se podría enunciar de forma análoga para $y$.
%\begin{enumerate}
...
%\end{enumerate}
\end{theo}
Para la demostración de este resultado, se hará uso de los siguientes lemas, ya probados anteriormente, que se proceden a enunciar de nuevo.
\begin{lema}
Si $f, g$ satisfacen el teorema de Tonelli, entonces su combinación lineal positiva, es decir, $a\cdot f + b \cdot g : a,b \in \mathbb{R}^+$, también satisface el teorema de Tonelli.
\end{lema}
\begin{lema}
Si $\{f_k\}_{k=1}^{\infty}$ es una sucesión de funciones que satisfacen el Teorema de Tonelli y $f_k \uparrow f$ puntualmente, entonces $f$ también satisface el Teorema de Tonelli.
\end{lema}
\begin{demo}[Teorema de Tonelli]
Procedemos a la demostración del Teorema de Tonelli. Esta se realiza de forma gradual, considerando funciones sencillas que en conjunto harán cierto el resultado para cualquier función medible $f$.

\begin{enumerate}
\item Primeramente consideramos $f = \chi_{Q}$ donde $Q \subset \mathbb{R}^{n+m}$ es un cubo semiabierto, es decir, $Q = \prod_{i=1}^{n+m} \left[a_i, b_i\right)$. Así, podemos considerar el cubo $Q$ como el producto cartesiano $Q = A \times B$ con $A \subset \mathbb{R}^n$ y $B \in \mathbb{R}^m$ cubos semiabiertos. Veamos que $f$ cumple el Teorema de Tonelli.
\begin{itemize}
    \item $\left(\chi_Q\right)_x = \chi_{Q_x} = \begin{cases}
        \chi_B \text{, si } x \in A\\
        0 \text{, si } x \not\in A
    \end{cases} $. Es decir, $\left(\chi_Q\right)_x $ es medible  $\forall x \in \mathbb{R}^n$.
    \item $F \left(x\right) = \int_{\mathbb{R}^n} \chi_{Q_x} \dif{\mu_n} = \begin{cases}
        ...
        \left(\text{si } x \not\in A\right) = 0
    \end{cases}$ es medible. En particular
...
\end{itemize}
\item Consideramos ahora $f = \chi_G$ donde $G \subset \mathbb{R}^{n+m}$ es un conjunto abierto. Escribimos $G = \bigcup_{j=1}^{\infty} Q_j$ donde cada $Q_j$ es un cubo semiabierto y la unión es disjunta.

Denotando $G_k = \bigcup_{j=1}^{k} Q_j$ tenemos que $\chi_{G_k} = \sum_{j=1}^{k} \chi_{Q_j}$. Como $\chi_{G_k} $es una suma finita de funciones características de cubos, resulta que cumple el Teorema por el primer lema. Pero, $\{\chi_{Q_k}\}_{k=1}^{\infty} \uparrow \chi_G$ 
$$\chi_G = \sum_{j=1}^{\infty} \chi_{G_j} = \lim_{k \rightarrow \infty}\chi_{G_k} $$Así, aplicando el segundo lema resulta que $\chi_G$ cumple el Teorema de Tonelli.

\item Supongamos que $f = \chi_D$ donde $D \subset \mathbb{R}^{n + m}$ es un conjunto $G\delta$. Puesto que $D$ es una unión creciente $$D = \bigcup_{k=1}^{\infty} D_k$$ donde cada $D_k$ es $G\delta$ y acotado (por ejemplo, $D_k = D \cap \left(-k, k\right)^{n+m}$).

al ser $\{\chi_{D_k}\}_{k=1}^{\infty} \uparrow \chi_D$, usando el segundo lema, es suficiente considerar el caso de que $D$ es acotado y $G\delta$. 

Así, supongamos que $D$ es $G\delta $ y acotado, escribimos $$D = \bigcap_{k = 1}^{\infty} G_k$$ donde cada $G_k$ es abierto acotado y $G_1 \supset G_2 \supset G_k \supset \ldots \supset D$. Entonces, $\{\chi_{G_k}\}_{k=1}^{\infty} \downarrow \chi_D$. Usando la versión decreciente de TCM, se puede hacer una demostración análoga a la del segundo lema para obtener que $\chi_D$ verifica el teorema. 

Alternativamente, como $0 \le \chi_D \le \chi_{G_1}$ es integrable y satisface el teorema y se puede aplicar también el TCD.

\item Consideramos ahora $f = \chi_N$ donde $\mu_{n+m} \left(N\right) = 0$. Por la regularidad de la medida, 
$$\forall k \in \mathbb{N},\ \exists G_k \mbox{ abierto en }\mathbb{R}^{n}\times \mathbb{R}^m \mbox{ con } N \subset G_k  \mbox{ y }  \mu_{n+m} \left(G_k \setminus N\right) < \frac{1}{k}$$

Sea $G = \bigcap_{k \in \mathbb{N}} G_k$ . $G$ es un $G\delta$ por se intersección numerable de abiertos tal que $N \subset G $ y 
$$\mu_{n+m} \left(G\setminus N\right) \le \mu_{n+m} \left(G_k \setminus N\right) < \frac{1}{k} \rightarrow 0$$
...
Entonces, $\chi_G$ satisface el teorema por el apartado anterior para casi todo $x \in \mathbb{R}^n$, y por tanto $\chi_G$ es medible, 
...
Por último veamos que son iguales las siguientes expresiones: 
$$\int_{\mathbb{R}^n \times \mathbb{R}^m} \chi_N \dif{\mu_{n+m}} = \int_{\mathbb{R}^n} \left(\int_{\mathbb{R}^m} \chi_{N_x} \dif{\mu_m} \right) \dif{\mu_n} $$

Vemos que 
$$\int_{\mathbb{R}^n \times \mathbb{R}^m} \chi_N \dif{\mu_{n+m}} = \mu_{n+m} \left(N\right) = 0$$.

Y además
$$\int_{\mathbb{R}^n} \left(\int_{\mathbb{R}^m} \chi_{N_x} \dif{\mu_m} \right) \dif{\mu_n} = \int_{\mathbb{R}^n} F \left(x\right) \dif{\mu_n} = \int_{\mathbb{R}^n} 0 \dif{\mu_n} =0$$
...
\item Sea ahora $f = \chi_A$ donde $A \subset \mathbb{R}^{n+m} = \mathbb{R}^n \times \mathbb{R}^m$ es medible. 

Sabemos que podemos descomponer $A$ como $A = D \setminus N$, donde $D$ es un $G\delta $ y $\mu_{n+m} \left(N\right) = 0$. Entonces, $D = A \sqcup N : \chi_D = \chi_A + \chi_N$. Así,
...
puesto que $\mu_m \left(N_x\right) = 0$ y, consecuentemente, $\int_{\mathbb{R}^m} \chi_{N_x} \dif{\mu_m} = 0$. Por tanto, es medible para casi todo punto $x \in \mathbb{R}^n$ porque $D$ es $G_\delta$ y aplicamos el apartado (3).
...
\item Sea $f$ función simple, medible, no negativa. Entonces, $f$ es combinación lineal no negativa de funciones características de conjuntos medibles en $\mathbb{R}^n \times \mathbb{R}^m$ Por el primer lema y (5), obtenemos que $f$ satisface el teorema. 

\item Finalmente, consideramos $f$ medible no negativa. Entonces $ \exists \{f_k\}_{k=1}^{\infty}\uparrow f$ donde cada $f_k$ es simple, medible y no negativas. Por el lema $2$ y (6), $f$ satisface el teorema.
\end{enumerate} 
\end{demo}
