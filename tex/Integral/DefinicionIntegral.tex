...
\end{prop}
\begin{demo}
Supongamos que $\sum_{i=1}^{m} \alpha_i \chi_{E_i}$ es la forma canónica de $\varphi$, que definimos por aquella en la que las $\alpha_i$ son todas distintas y las $E_i$ son disjuntas. Veamos que es posible llegar a esta expresión desde cualquier otra manteniendo las medidas.

Partiendo de $\sum_{i=1}^{p} \beta_i \chi_{D_i}$ no canónica, sabemos que es posible que:
\begin{itemize}
    \item $\beta_i$ sean distintos.
    \item $E_i$ sean disjuntos.
\end{itemize}
Empecemos ``separando'' los $E_i$. Para ello observamos que si no son disjuntos es porque existen elementos que pertenecen a la intersección de algunos. Estos elementos tendrán como imagen la suma de los $\beta_i$ asociadas a los conjuntos que forman esta intersección. 

Con esto, podemos definir el conjunto disjunto formado por los $E_i$ eliminándoles los $x$ que pertenecen a varios, pero añadiéndolos a través de las intersecciones con el $\beta_i'$ asociado formado por la suma anterior. Esto mantiene la medida porque, si $B \subset A \Rightarrow$ 
\[
\mu \left( A \setminus B \right) = \mu \left( A \right) - \mu \left( B \right) \Rightarrow \boxed{\mu \left( A \right) = \mu \left( A \setminus B \right) + \mu \left( B \right)}
\]
Es decir, si inicialmente tenemos $\left\{ E_i \right\}_1^m$ y definimos:
\[
    \begin{cases}
        E_i' = E_i \setminus \left( \bigcup_{j \neq i} E_i \cap E_j \right),\ 1 \le i \le m\\ 
        E_k' = E_i \cap E_j,\ i \neq j
    \end{cases}
\]
Los $E_k'$ serán finitos al serlo los $E_i$. Así, los $E_i'$ serán disjuntos y si aplicamos recursivamente el mismo procedimiento a los $E_k'$ nos quedarán también disjuntos en un número finito de pasos.

Separemos ahora las $\beta_i$, es decir, busquemos una nueva familia que proceda de la que acabamos de formar de tal forma que se mantenga la medida y los coeficientes sean todos distintos. De esta forma, ya tendremos una forma canónica con la misma integral.

Por tanto, si tomamos todas las $\beta_i$ con el mismo valor y los nuevos conjuntos asociados, podemos definir un nuevo conjunto $A_i = \bigcup_{i = 1}^t E_i$ que mantendrá la medida porque: 
\[
\mu \left( \bigsqcup_{i = 1}^t E_i \right) = \sum_{i=1}^{t} \mu \left( E_i \right) 
\]
y los $\gamma_k = \sum_{i=1}^{t} \alpha_i$ serán todos distintos, es decir, 
\[
\sum_{i=1}^{r} \gamma_i A_i 
\]
será la forma canónica que tendrá la misma medida que la forma inicial. 

Para comparar dos formas no canónicas, simplemente las pasamos a la canónica y vemos que son iguales.
\end{demo}
...
\end{prop}
\begin{demo}
Buscamos ver si $\sup I \left( \varphi \right) \le \sup I \left( \psi \right)$ donde $\varphi \le f$ y $\psi \le g$. Como $\varphi \le f \le g$ y $\psi \le g \Rightarrow$ los $\varphi$ son un subconjunto de los $\psi$ de tal modo que:
\[
\exists \psi' \le g \text{ tal que } \forall \varphi \le f, \varphi \le \psi \Rightarrow I \left( \varphi \right) \le I \left( \psi' \right) \Rightarrow \sup I \left( \varphi \right) \le \sup I \left( \psi \right) 
\]
Para el c.t.p usamos que la integral en ese conjunto de medida $0$ es $0$ por lo que la integral en todo $X$ es la integral en el complementario de dicho conjunto.
\end{demo}
...
\begin{theo}
Sea $f: \left[a, b\right] \rightarrow \mathbb{R}^+$ una función acotada e integrable Riemann, entonces $f$ es integrable Lebesgue y verifica:
...
\end{prop}
\begin{demo}
En primer lugar, sea $k \ge 1$. Dividimos la imagen de la función en intervalos disjuntos $I_j = [(j-1) \cdot 2^{-k}, j \cdot 2^{-k}]$ donde $j = 1, \ldots, k \cdot 2^k$ y la unión de todos los intervalos resulta el intervalo $[0,k)$
$$\begin{tikzpicture}
\begin{axis}[
axis y line=middle,
axis x line=middle,
...
\end{tikzpicture}$$
%TODO: Corregir? añadiendo E_k = {f(x) > k}
...
Definimos la siguiente función simple, finita por serlo los $E_k^j$:
...
\begin{align*}
\frac{j_{k, x} - 1}{2^k} \le f&\left( x \right) \le \frac{j_{k, x}}{2^k} \Rightarrow\\
0 < f\left(x \right) - \varphi_k \left(x\right) < \frac{1}{2^k} &\Rightarrow \boxed{\lim_{k \rightarrow \infty} \varphi_k\left(x\right) = f\left(x\right)}
\end{align*}
\end{demo}
...
Recordemos que: 
\[
\sum_{i=1}^{N} f\left( x \right) \le \sum_{i=1}^{N+1} f\left( x \right) \text{ y } \lim_{n \rightarrow \infty} f \left( x \right) = f \left( x \right) 
\]
Por tanto, la demostración es prácticamente consecuencia inmediata de los enunciados anteriores y del teorema de convergencia monótona:
...
Esta última desigualdad porque al ser $g_k\left( x \right) = \inf_{m \ge k} f_m\left( x \right) \Rightarrow$
\[
g_k\left( x \right) \le f_k\left( x \right)\Rightarrow \int_{\mathbb{R}^n} g_k\left( x \right) \mathrm{d}\mu \le \int_{\mathbb{R}^n} f_k\left( x \right) \mathrm{d}\mu
\]
\end{demo}

\underline{Ejercicio}: Ver que el lema de Fatou implica el teorema de convergencia monótona.

\begin{ej}
Sea $f_k \left( x \right) = k \chi_{\left[ 0, \frac{1}{k} \right]},\ k \ge 1$. Ver como la desigualdad es estricta.
\end{ej}
