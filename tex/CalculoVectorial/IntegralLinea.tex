...
%TODO: Arreglar
\begin{defi}
Llamamos \textbf{dominio proyectable} si existen funciones continuas $y = \alpha\left( x \right)$, $y = \beta\left( x \right),\ \alpha\left( x \right) \le \beta\left( x \right)$ definimos en un intervalo $\left[ a, b \right]$ tal que $\alpha\left( x \right) < \beta\left( x \right),\ x \in \left( a, b \right)$ a:
\[
D = \left\{ \left( x, y \right): x \in \left[ a, b \right] \text{ e } \alpha\left( x \right) \le y \le \beta\left( x \right) \right\}
\]
\end{defi}
...
\end{theo}
\begin{obs}
Podemos escribir $\int_{\partial D^+} \left( P, Q \right) \mathrm{d}\overline{s}$ como:
\[
\oint_{\partial D^+} P\left( x, y \right) \dif{x} + Q\left( x, y \right) \dif{y}
\]
\end{obs}
\begin{demo}
...
Suponiendo ahora las funciones sobre $y$ que definen al conjunto, es decir, 
$$D = \left\{ \left( x, y \right) : y \in \left[ c, d \right] \; \land \; f\left( y \right) \le x \le g\left( y \right) \right\}$$
donde $f, g: \left[ c, d \right] \rightarrow \mathbb{R}$ son $C^1$ a trozos. La demostración es completamente análoga: 
$$\int_{\partial D^+} Q \dif{y} \overbrace{=}^{\text{ejercicio}} \iint_{D} \frac{\partial Q}{\partial x} \dif{x} \dif{y}$$
